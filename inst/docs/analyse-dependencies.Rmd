...
```{r}
# ESD public packages 
esd.public <- sort(c("sedproxy", "psem", "PaleoSpec", "orbitalforcing",
                "piccr", "cptPicarr", "TrenchR", "corit", "ecustools",
                "prxytools", "stattools", "grfxtools", "geostools", "ncdftools"))
```
...
```{r}
# wrapper function to do the work
GetFCalls <- function(package, 
                      level = c("package", "function"),
                      exclude_base_recommended = TRUE, 
                      exclude_self = TRUE){
  
  level <- match.arg(level)
  
  base.packs <- names(which(installed.packages()[ ,"Priority"] == "base", ))
  rec.packs <- names(which(available.packages(
    repos = c(CRAN = "https://cran.r-project.org"))[ ,"Priority"] == "recommended", ))
  
  all.calls <- data.frame(focal.package = package,
                          stringsAsFactors = FALSE) %>% 
    group_by(focal.package) %>% 
    do({
      itdepends::dep_usage_pkg(.$focal.package)
  ...
  if (exclude_base_recommended){
    all.calls <- all.calls %>%
      filter(pkg %in% c(base.packs, rec.packs) == FALSE)
  }
  
  if (exclude_self) {
    all.calls <- all.calls %>%
      filter(focal.package != pkg)
  }  
...
GetFCalls("esd.public", exclude_self = TRUE, level = "f")


GetFCalls("hamstr", exclude_self = TRUE, level = "p") 
